...
* Follow PEP8, except
  --ignore=E226,E265,E266,E401,E402,E501,W291,W293,W391

* Favor unmutable classes over mutable ones

* For arrays of numbers, use core/FracVector unless you have a
  reason

* Constructors are generally considered private, use a create(...)
...
* Arrays of numbers: essentially all arrays of numbers within
  ...
  primary reasons for this design choice was:
     ...
     * FracVectors are immutable (but there is a MutableFracVector).
       ...
       (For more info, see the section 'Rant about mutable vs. non-
       mutable classes' at the end of this document.)
...
* Basic structural classes: we implement our own, rather than using
  a 'structure' class of another library (e.g., 'Atoms' from ASE).
  This way we avoid dependencies, but most importantly, our structure
...
and inelegant if __init__ is already an established public swiss-army-
...
http://www.python.org/dev/peps/pep-0008/
   You are encouraged to use the pep8 tool (either directly or via
   your code development platform, but, use:
   --ignore=E401,E402,E501,W291,W293,W391,E265,E266,E226 (See below
   for motivations.)
...
Rule #2: Always organize your code in private sections and a public
   ...
   class / module / etc.
      ...
      into a huge pile of code that has such intricate cross-
      dependences that it is almost impossible to know the
      ...
      internal data structures and make assumptions about how they are
      organized.
      ...
      The principle of API-oriented organization is simple:
         ...
         * Changes to the public API are difficult, and should
           generally be done only by introducing a new version of the
           class / module / etc.
...
* W291: trailing whitespace: Between all different editors used,
  this simply generates too many warnings that makes more important
  pep8 violations more difficult to see.  Once in a while we should
  simply run the files through a tool that removes trailing
  whitespace.

* W293: blank line contains whitespace: I genuinely disagree with
  this rule. It is not motivated by the pep0008 standard, but
  something unmotivated put in by developers of the pep8 tool.  Blank
  lines should be indented to the indentation level of the block that
  they appear in.
...
If you plan on redistributing / forking httk with major changes,
PLEASE edit httk/__init__.py and change the 'version' variable to
contain a personal suffix.  E.g., set version='1.0.rickard.2'. Then
run the command 'make dist'. This creates a httk_v{VERSION}.tgz
archive that you can redistribute.
