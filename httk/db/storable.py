...
class Storable(object):
    ...
    def storable_init(self,store,**keyvals):
        """
        ...
        self.__dict__['store'] = store.new(self.types['name'],self.types,keyvals)
    ...
    @classmethod
    def variable(cls,searcher,name,outid=None):        
        ...
        searcher.store.new(cls.types['name'],cls.types)

        if outid == None:
            outid = name+"_"+str(len(cls._storable_tables))
        table = TableOrColumn(searcher, cls.types['name'],outid=outid,indirection=1,classref=cls)
        return table
    
    @classmethod
    def find_all(cls, store, member, value):        
        """
        Convinience method to do a very simple search of type: find all entries where member = value.
        """
        search = store.searcher()
        p = cls.variable(search,'p')
