...
def _cif_is_float(data_value):
    return (_cif_float_regex.match(data_value)!=None)

def _cif_is_simplestring(data_value):
    return (_cif_simplestring_regex.match(data_value)!=None)

def _cif_is_int(data_value):
    return (_cif_integer_regex.match(data_value)!=None)
...
def write_cif(ioa,data,header=None,max_line_length=80, use_types=False):
    ...
    for data_block in data:
        ...
        if data_block_name_unfiltered == None:
