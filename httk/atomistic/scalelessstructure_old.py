...
#                         rc_cellshapeobj = None
    ...
    @property
    def uc_formula_spaceseparated(self):
        parts = self.uc_formula_parts        
        formula = ''
        for symbol, count in parts:            
            count = FracScalar.use(count).set_denominator(100).simplify()
    ...
    io = HttkPluginPlaceholder("import httk.atomistic.atomisticio")
    vis = HttkPluginPlaceholder("import httk.atomistic.vis")


class SlStructureTag(HttkObject):                               

    @httk_typed_init({'structure': ScalelessStructure, 'tag': str, 'value': str}, index=['structure', 'tag', ('tag', 'value'), ('structure', 'tag', 'value')], skip=['hexhash'])    
