...
def readstruct(file_or_filename,importers=None):

    fileadapter = ioadapters.FileAdapter(file_or_filename)

    if fileadapter.ext == 'structure':
        structure.parse(fileadapter)
    ...
    for importer in try_importers:
        ...
        if importer == 'ase':
            try:
                ...
                return structure.Structure(basis=basis,coords=coords,species=species)
            except:
                if importers != None:
                    info = sys.exc_info()
