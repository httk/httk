...
def convert_to_simplestruct(struct_obj):
    ...
    print(cell.normalization_scale)
    print("\n     ".join(["% .8f % .8f % .8f" % (x[0], x[1], x[2]) for x in cell.get_normalized().basis.to_floats()]))
    print(assignments.symbollists)
    print("\n".join(["".join(["    %.8f %.8f %.8f\n" % (x[0], x[1], x[2]) for x in y]) for y in sites.reduced_coordgroups.to_floats()]))
    simple_obj = SimpleStructure(cell_basis=cell, sites_fractional=sites, species=assignments)
    return simple_obj