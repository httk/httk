...
def make_database(db_name):
    ...
    for rundir, computation in reader:
        cluster = re.search(".*ht\.task\.(\w*)--.*", rundir)
        ...
        # Record which cluster was used
        computation.add_tag('cluster', cluster)

        with open(os.path.join(root, "job_info.json"), "r") as f:
            jobid = json.load(f)['struct_hash']

        outcar = httk.iface.vasp_if.read_outcar(os.path.join(rundir, "OUTCAR.cleaned.relax-final"))
        result = Result_ElasticResult(computation, initial_struct, struct,
                float(outcar.final_energy),
                cij, cij_nosym, sij, elas_dict['K_V'], elas_dict['K_R'],
                elas_dict['K_VRH'], elas_dict['G_V'],
                elas_dict['G_R'], elas_dict['G_VRH'],
                elas_dict['mu_VRH'], elas_dict['E_VRH'],
                atomic_relaxations, jobid)
